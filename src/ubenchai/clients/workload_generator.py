...
class WorkloadGenerator:
    ...
    def __init__(
        self, target_endpoint: str, model_name: str = "tinyllama", timeout: int = 30
    ):
        """
        ...
        logger.info(
            f"WorkloadGenerator initialized: {self.target_endpoint}, model={self.model_name}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.model_name in models:
                    return (
                        True,
                        f"Connection OK. Model '{self.model_name}' is available.",
                    )
                else:
                    return (
                        False,
    ...
    def run_closed_loop(
        ...
        """
        logger.info(
            f"Starting closed-loop workload: {concurrent_users} users, {duration_seconds}s, model={self.model_name}"
        )
    ...
    def run_open_loop(
        self, duration_seconds: int, requests_per_second: int, prompt_length: int = 50
    ) -> WorkloadResult:
        """
        ...
        """
        logger.info(
            f"Starting open-loop workload: {requests_per_second} RPS, {duration_seconds}s, model={self.model_name}"
        )
...
def main():
    ...
    generator = WorkloadGenerator(target_endpoint=args.endpoint, model_name=args.model)
