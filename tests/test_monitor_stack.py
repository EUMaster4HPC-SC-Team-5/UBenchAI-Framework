...
def test_monitoring_stack_deployment(temp_workspace):
    ...
    if instance.prometheus_url is not None:
        ...
        logger.info("Prometheus not available in test environment - testing fallback behavior")
...
def test_monitoring_stack_with_mock_binary(temp_workspace, monkeypatch):
    ...
    monkeypatch.setattr("ubenchai.monitors.prometheus_client.PrometheusClient._which", mock_find_prometheus_binary)
    monkeypatch.setattr("ubenchai.monitors.grafana_client.GrafanaClient._which", mock_find_grafana_binary)
